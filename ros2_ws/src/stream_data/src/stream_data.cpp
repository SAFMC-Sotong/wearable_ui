...
public:
  StreamData() : Node("stream_data")
  {
    ...
    battery_sub_ = this->create_subscription<mavros_msgs::msg::BatteryStatus>(
        "/mavros/battery", 10, std::bind(&StreamData::battery_callback, this, std::placeholders::_1));
...
private:
  ...
  void battery_callback(const mavros_msgs::msg::BatteryStatus::SharedPtr msg)
  {
    battery_msg_ = *msg;
  ...
  void udp_send()
  {
    ...
    {
    std::string battery_msg = std::to_string(battery_msg_.percentage);
      ...
      {
        RCLCPP_INFO(this->get_logger(), "Battery: %d%%", battery_msg_.percentage);
        battery_update_ = false;  
      }
    }
    if (mode_update_ && mode_socket_ >= 0)
    {
    std::string mode_msg = mode_msg_.mode;
      ...
      {
        RCLCPP_INFO(this->get_logger(), "Mode: %s", mode_msg_.mode);
        mode_update_ = false;
      }
    }
  }
  ...
  rclcpp::Subscription<mavros_msgs::msg::BatteryStatus>::SharedPtr battery_sub_;
  ...
  mavros_msgs::msg::BatteryStatus battery_msg_;
